                ...
                {partidosFiltrados.map((partido) => {
                  ...
                  return (
                    <div key={partido.id} className="bg-gray-950/70 p-4 rounded-xl border border-gray-800/80 shadow-sm flex flex-col">
                      ...
                      {/* Cuadrícula de Equipos y Marcador (Sin botones de guardado) */}
                      ...
                      {/* Fila Independiente para Botones de Acción */}
                      <div className="mt-3 pt-4 border-t border-gray-800/60 w-full">
                        {partido.estado === 'pendiente' ? (
                          <button 
                          ...
                          <button 
                            ...
                            className="text-xs uppercase tracking-wider bg-gray-800 hover:bg-gray-700 text-gray-300 px-4 py-2.5 rounded-lg w-full font-bold transition border border-gray-700 shadow-sm"
                          >
                            👀 Ver Pronósticos
                          </button>
                        )}
                    ...
                    {equipos.map((equipo) => (
                      <tr key={equipo.id} className={`transition-colors ${obtenerColorFila(equipo.posicion_real_actual)}`}>
                        ...
                        <td className="py-3 px-3 flex items-center gap-3">
                          {equipo.escudo_url ? <img src={equipo.escudo_url} alt={`Escudo`} className="w-6 h-6 object-contain shrink-0" /> : <div className="w-6 h-6 bg-gray-800 rounded-full shrink-0"></div>}
